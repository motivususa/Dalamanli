...
export const SOUNDS_MUTED_KEY = "ipodSoundsMuted";
...
export const useSettings = (): SettingsHook => {
  ...
  const setSoundsMuted = useCallback(
    (muted: boolean) => {
      setState((prevState) => ({ ...prevState, soundsMuted: muted }));
      localStorage.setItem(SOUNDS_MUTED_KEY, muted ? "true" : "false");
    },
    [setState]
  );
...
export const SettingsProvider = ({ children }: Props) => {
  ...
  const handleMount = useCallback(() => {
    setSettingsState((prevState) => ({
      ...
      soundsMuted: localStorage.getItem(SOUNDS_MUTED_KEY) === "true",
    }));
  }, []);
